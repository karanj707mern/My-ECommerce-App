...
describe('HealthController', () => {
  ...
  it('should return ready 200 when all checks are ok', async () => {
    ...
    await controller.ready(response as unknown as FastifyReply);

    expect(response.status).toHaveBeenCalledWith(200);
    expect(response.send).toHaveBeenCalledWith({ status: 'ready' });
  });
  ...
  it('should return not_ready 503 when checks are degraded', async () => {
    ...
    expect(response.send).toHaveBeenCalledWith({ status: 'not_ready' });
  });
