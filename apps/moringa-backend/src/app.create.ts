...
export interface CreateAppOptions {
  /**
   * Attach the Socket.IO Redis adapter. Enabled by default in production;
   * disable in integration tests to keep them hermetic (no Redis required).
   */
  websocket?: boolean;
  /** Register Swagger docs. Defaults to development NODE_ENV only. */
  swagger?: boolean;
}

/**
 * Single source of truth for application assembly.
 *
 * Both `main.ts` and integration/e2e tests build through this factory, which
 * guarantees tests exercise EXACTLY the production pipeline: same BYO Fastify
 * instance, plugin registration order, global prefix, filters, interceptors,
 * pipes, CORS policy, and Swagger surface.
 *
 * ARCHITECTURE NOTES (load-bearing decisions):
 *
 * 1. BYO-instance pattern (`new FastifyAdapter(server)`): every @fastify/*
 *    plugin is registered BEFORE Nest touches the server, so avvio boots them
 *    in order prior to route registration. This avoids post-boot plugin
 *    registration issues and the FastifyTypeProvider generic mismatches
 *    between plugin typings and the adapter's vendored fastify copy.
 *
 * 2. `rawBody: true`: the adapter's JSON parser captures untouched payload
 *    bytes at req.rawBody — required for Razorpay webhook HMAC verification.
 */
export async function createApp(options: CreateAppOptions = {}): Promise<{
  ...
  const wantSwagger =
    options.swagger ?? (!process.env.NODE_ENV || process.env.NODE_ENV === 'development');
  if (wantSwagger) {
