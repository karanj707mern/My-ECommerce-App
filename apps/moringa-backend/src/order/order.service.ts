...
export class OrderService {
    ...
    for (const order of expiredOrders) {
      await this.prisma.$transaction(async (tx) => {
        ...
        if (freshOrder.inventoryReserved) {
          await this.restoreOrderStock(tx as unknown as PrismaService, freshOrder.items);
        }
  ...
  streamOrders(userId: number): Observable<OrderEventMessage> {
    return new Observable<OrderEventMessage>((subscriber) => {
      subscriber.next({
        type: 'order.updated',
        orderId: 0,
        userId,
      ...
      const { unsubscribe } = this.orderEventsService.onUpdate((update) => {
        if (update.userId === userId) {
          subscriber.next(update.message);
        }
      });
  ...
  streamAdminOrders(): Observable<OrderEventMessage> {
    return new Observable<OrderEventMessage>((subscriber) => {
      subscriber.next({
        type: 'order.updated',
        orderId: 0,
  ...
  private buildOrderWhereClause(userId: number | undefined, query: QueryOrderDto) {
