...
/**
 * Socket.IO adapter backed by Redis pub/sub so order events broadcast through
 * the gateway reach clients connected to ANY backend instance. Falls back to a
 * single-node server when REDIS_URL is not configured.
 */
export class RedisIoAdapter extends IoAdapter {
  ...
  private startHealthCheck(): void {
    ...
    // Keep the key referenced so linters do not flag an unused field; the key
    // is reserved in Redis for future external probes.
    void this.healthCheckKey;
  }
