import type { Metadata } from "next";
import { getFirstActiveHeroImage } from "@/lib/api/hero";
import InfoPage from "@/components/InfoPage";

export const revalidate = 300;

export async function generateMetadata(): Promise<Metadata> {
  const siteUrl =
    process.env.NEXT_PUBLIC_SITE_URL?.replace(/\/$/, "") ||
    "https://my-nest-project-pearl.vercel.app";

  let heroImage =
    "https://my-nest-project-pearl.vercel.app/images/home-hero-1.webp";

  try {
    const firstHero = await getFirstActiveHeroImage();
    if (firstHero?.url) {
      heroImage = firstHero.url;
    }
  } catch {
    // Keep fallback on error
  }

  return {
    title: {
      default: "Privacy Policy",
      template: "%s | Moringa Store Online",
    },
    description:
      "Review how Moringa Store Online handles account, order, and customer information.",
    alternates: { canonical: "/privacy-policy" },
    openGraph: {
      type: "website",
      locale: "en_IN",
      siteName: "Moringa Store Online",
      title: "Privacy Policy | Moringa Store Online",
      description:
        "Review how Moringa Store Online handles account, order, and customer information.",
      images: [
        {
          url: heroImage,
          width: 1200,
          height: 630,
          alt: "Privacy Policy - Moringa Store Online",
        },
      ],
      url: `${siteUrl}/privacy-policy`,
    },
    twitter: {
      card: "summary_large_image",
      title: "Privacy Policy | Moringa Store Online",
      description:
        "Review how Moringa Store Online handles account, order, and customer information.",
      images: [heroImage],
    },
  };
}

export default function PrivacyPolicyPage() {
  return <InfoPage pageKey="privacy-policy" />;
}
